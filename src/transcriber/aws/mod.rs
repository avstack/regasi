...
use gstreamer::{
  ...
  Bin, Buffer, Caps, Element, ElementFactory, FlowError, FlowSuccess, GhostPad, PadDirection,
...
impl AwsTranscriber {
  async fn transcribe(
    ...
    let (audio_stream, abort_audio_stream) = {
      ...
      stream::abortable(
        ReceiverStream::new(audio_rx)
          .map(move |audio| {
            debug!(
              "encoding {} bytes of audio to eventstream message",
              audio.len()
            );
            Ok(Some(
              Message::new(audio)
            ...
            async move {
              let message = {
                ...
                let (message, sig) = if let Some(payload) = maybe_payload {
                  ...
                  debug!("prev signature: {}", last_sig_locked);
                  aws_sigv4::event_stream::sign_empty_message(&*last_sig_locked, &signing_params)
                    .into_parts()
                };
    ...
    let (sig_headers, sig) = calculate_signing_headers(
      &req,
            ...
            match serde_json::from_str::<AwsTranscriptEvent>(payload_str) {
              Ok(event) => {
                ...
                for result in event.transcript.results {
                  ...
                  transcription_tx
                    .send(TranscriptionEvent::Speech {
                      timestamp: Utc::now(),
                      participant: participant.clone(),
                      is_interim: result.is_partial,
                      stability: if alternative.items.iter().all(|item| item.stable) {
                        1.0
                      }
                      else {
                        0.9
...
impl Transcriber for AwsTranscriber {
  fn new(
    ...
    let join_handle = tokio::spawn(async move {
      if let Err(e) =
        AwsTranscriber::transcribe(region, participant, audio_rx, transcription_tx).await
      {
  ...
  fn make_element(&self) -> Result<Element> {
    ...
    let opusparse =
      ElementFactory::make("opusparse", None).context("failed to create opusparse element")?;
    bin.add(&opusparse)?;
    ...
    let oggmux = ElementFactory::make("oggmux", None).context("failed to create oggmux element")?;
    bin.add(&oggmux)?;
    ...
    appsink.set_callbacks(
      AppSinkCallbacks::builder()
        .new_sample(move |appsink| {
          let sample = appsink.pull_sample().map_err(|_| FlowError::Error)?;
          if let Some(buffer) = sample.buffer() {
            let mut raw_audio = BytesMut::with_capacity(buffer.size());
            raw_audio.resize(buffer.size(), 0);
            buffer.copy_to_slice(0, &mut raw_audio).map_err(|_| {
              error!("failed to copy buffer");
              FlowError::Error
            })?;
            audio_tx
              .blocking_send(raw_audio.freeze())
